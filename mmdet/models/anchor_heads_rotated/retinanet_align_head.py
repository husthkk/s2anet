...
class RetinaAlignHeadRotated(nn.Module):
    ...
    def forward_single(self, x, stride):
        ...
        # refine_anchor = refine_anchor.reshape(bs, -1, 5)
        out_bboxes = bbox_decode(
            odm_bbox_pred.detach(),
            refine_anchor.reshape(bs, -1, 5),
            self.target_means,
            self.target_stds)

        return fam_cls_score, fam_bbox_pred, refine_anchor, odm_cls_score, odm_bbox_pred, out_bboxes
